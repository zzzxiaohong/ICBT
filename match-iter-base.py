...
def write_file(list, file_name):
    with open(file_name, 'w', encoding='utf-8') as f:
        for i in list:
            f.write(i + '\n')
def match(filepatch, fin_name, fout_name, src_lang, fdict_name, fout_path, max_num, devide=' '):
    fsrc_name = filepatch + '/' + fin_name
    with open(fsrc_name, 'r', encoding='utf-8') as fsrc,\
        ...
        src_term_list = [term.split(devide)[1] for term in term_dict]
        tgt_term_list = [term.split(devide)[0] for term in term_dict]
        matched_src_lines = []
        term_lines = []
                ...
                if src_term in src_line:
                    ...
                    for word in term_line:
                        ...
                        if src_term in word.split(' ||| '):
                            flag = False
                    if term not in term_line and flag == True:
                        term_line.append(term)
            random.shuffle(term_line)
            term_line = term_line[:max_num]
            matched_src_lines.append(src_line)
            term_line.sort(key=lambda i: len(i), reverse=True)
            term_lines.append('\t'.join(term_line))
        fsrc_out_name = fout_path + '/' + fout_name + '.match.' + src_lang
        write_file(matched_src_lines, fsrc_out_name)
        write_file(term_lines, fout_path + '/' + fout_name + '.term_lines')
...
def creat_batch(fin_name, pool_num):
    fsrc_name = fin_name
    with open(fsrc_name, 'r', encoding='utf-8') as fsrc:
        fsrc_all_lines = fsrc.readlines()
        print(len(fsrc_all_lines))
        ...
        for i in range(pool_num):
            ...
            fsrcout = open(fin_name + str(i+1), 'w', encoding='utf-8')
...
if __name__ == '__main__':
    ...
    creat_batch(args.fin_path+ '/' +args.fin_name, args.pool_num)
