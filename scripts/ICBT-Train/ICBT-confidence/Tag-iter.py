...
def test_replace_by_tag(filepath, filename, sl, tl):
    with open( "%s/%s.match.%s"%(filepath, filename, sl), 'r', encoding='utf-8') as fsrc,\
        ...
        assert len(src_res[1]) == len(src_lines),\
             "len(src_res[1]):%d, len(src_lines):%d"%(len(src_res[1]), len(src_lines))

        for i in range(1, 2):
            print("%s/%s.tag%s.%s"%(filepath, filename, i, sl))
